...
namespace COP.API.DatabaseInterface
{
    public class DBPersonWithWearablesApi
    {
        ...
        public bool ListPersonWithWearables(string role, ref string errorMessage, ref List<COP.API.Models.PersonWithWearable> results)
        {
            bool retVal = true;
            try
            {
                using (COPDBContext.monica_cnetContext context = new COPDBContext.monica_cnetContext())
                {
                    ...
                    if (pp == null || pp.Count() == 0)
                    {
                        ...
                        foreach (var p in pp)
                        {
                            COP.API.Models.PersonWithWearable z = new Models.PersonWithWearable();
                            ...
                            foreach (var obs in wearable)
                            {
                                ...
                                if (strTime == null)
                                    strTime = DateTime.Now.ToString("O");
        ...
        public bool ListPersonByIdWithWearables(int personId, ref string errorMessage, ref COP.API.Models.PersonWithWearable result)
        {
            bool retVal = true;
            try
            {
